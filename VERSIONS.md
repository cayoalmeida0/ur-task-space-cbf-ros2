## Revisao experimental 0.6.31 / controle 0.6.31 / bringup 0.3.14 — 15 de setembro de 2026

- Reduz a altura de aproximacao de 0,10 m para 0,05 m, mantendo a mesa em 0,15 m, o cubo em z=0,17 m e a elevacao pos-pega em 0,22 m.
...
- Expõe `cube_position`, `drop_position` e `manipulation_object_frame` como
...
- Aumenta a aproximação vertical do cubo para `0,20 m` e a elevação após a
  pega para `0,22 m`, reduzindo o risco de contato da garra com a mesa.
...
- Define `z=0,050 m` em `c41` e `x=0` em `c42`, que permanece com `z=0`.
- Aplica os mesmos ajustes às coordenadas DH do UAIbot, preservando todos os
  outros origins, tipos, dimensoes, rotacoes, capsula RG2 e imagem Docker `0.2.0`.

## Revisao experimental 0.6.11 / controle 0.6.11 / bringup 0.3.11 — 26 de agosto de 2026

- Define no frame URDF `z=0,115 m` em `c11` e `z=0,050 m` em `c21`.
- Define `z=0,050 m` em `c31`, `y=0,025 m` em `c32` e `z=0,010 m` em
  `c41`, preservando os demais eixos desses objetos.
- Define `z=-0,018 m` em `c52`; preserva `c22`, `c23`, `c42`, `c51`, tipos,
  dimensoes, rotacoes e a capsula RG2.
- Aplica as conversoes equivalentes aos frames DH do UAIbot e mantém a imagem
  Docker `0.2.0`.

## Revisao experimental 0.6.10 / controle 0.6.10 / bringup 0.3.10 — 26 de agosto de 2026

- Define no frame URDF `z=0` em `c11`, `z=0,030 m` em `c21` e
  `z=0,027 m` em `c22`, preservando `z=0,025 m` em `c23`.
- Define `z=-0,020 m` em `c31`, `y=0,020 m` em `c32`, `z=-0,010 m` em
  `c41` e `z=0` em `c42`; preserva os demais eixos desses objetos.
- Mantém `c51` inalterado e preserva `z=-0,015 m` em `c52`.
- Aplica as conversoes equivalentes aos frames DH do UAIbot, mantendo tipos,
  dimensoes, rotacoes, capsula RG2 e imagem Docker `0.2.0`.

## Revisao experimental 0.6.9 / controle 0.6.9 / bringup 0.3.9 — 26 de agosto de 2026

- Define no frame URDF `z=0,020 m` em `c21`, `z=0,0225 m` em `c22` e
  preserva `z=0,025 m` em `c23`.
- Define `x=y=0; z=0,020 m` em `c31` e preserva `x=y=0` em `c32`.
- Preserva `c51` em `xyz=(0; 0; -0,020) m` e define `z=-0,015 m` em `c52`.
...
## Revisao experimental 0.6.8 / controle 0.6.8 / bringup 0.3.8 — 26 de agosto de 2026

- Preserva `z=0,025 m` em `c21/c22/c23` e `x=0` em `c21`.
- Retorna `c31` para `y=z=0` e `c32` para `y=0` no frame URDF.
- Define o origin de `c51` como `xyz=(0; 0; -0,02) m` e `z=-0,018 m` em `c52`.
- Aplica as mesmas poses ao Xacro e às matrizes DH do modelo UAIbot, preservando
  tipos, dimensoes, rotacoes, capsula RG2 e imagem Docker `0.2.0`.
...
## Revisao experimental 0.6.18 / controle 0.6.18 / bringup 0.3.14 — 11 de setembro de 2026

- acrescenta `link_2_obj_2__link_4_obj_0` (`C23` × `C41`) à matriz de
  colisões permitidas;
